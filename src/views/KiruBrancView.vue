...
<template>
  <div class="kiruBR" v-if="chars.length > 0">

    <div class="popup" v-if="dialog" >
      <p @click="showPopup" style="text-align: right; font-size: 1vw; font-family: LaborUnion, sans-serif; width: 97%;  margin-top: 1vw; margin-left: -6vw; position: relative">X <br> </p>
      <h1>{{chars[idchar].name}}</h1>
      <p v-html="chars[idchar].desk"></p>
      <img :src="chars[idchar].img" alt="" style="width: unset; height: 80%; position: absolute; right: 1vw; top: 1vw">
    </div>

    <img src="/Kazzy.png" alt="">
    <p>Ветка Кирю</p>
    <p class="goro"><router-link to="/chars/Majima">Перейти к Маджиме</router-link></p>
